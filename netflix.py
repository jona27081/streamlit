...
if st.sidebar.checkbox('Mostrar todo'):
...
if (btnBuscar):
   ...
   st.write(f"Total de peliculas encontradas : {count_row}")
   st.write(data_filme)


selected_director = st.sidebar.selectbox("Seleccionar director", data['director'].unique())
btnFilterbyDirector = st.sidebar.button('Filtrar')
